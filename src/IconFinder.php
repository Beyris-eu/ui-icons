...
class IconFinder implements ContainerInjectionInterface, IconFinderInterface {
   ...
   */
  private const LIMIT_SEARCH_EXT = ['gif', 'svg', 'png'];
  ...
  private function getFilesFromPath(string $source, string $drupal_root, string $definition_absolute_path, string $definition_relative_path): array {
    ...
    $finder = new Finder();
    try {
      $finder->depth(0)->in($path_search)->files()->name($names);
    }
    catch (\Throwable $th) {
    ...
    foreach ($finder as $file) {
      ...
      $count[$filename] = $count[$filename] ?? 0;
      if (isset($result[$filename])) {
        $count[$filename]++;
        $filename .= '__' . $count[$filename];
      }
      ...
      $icon_id = $this->getCleanIconId($filename) ?? $filename;
      if ($has_icon_pattern) {
        $icon_id = $this->determineIconId($path_info_filename, $icon_id);
      }
   ...
   */
  private function determineIconId(string $mask, string $filename): ?string {
   ...
   */
  private function getCleanIconId(string $name): ?string {
